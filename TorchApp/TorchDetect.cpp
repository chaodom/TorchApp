...
namespace torchapp {
	...
	vector<DetectOutput> TorchYoloV5::processOne(at::Tensor* pPredict, Mat origin_img, double scoreThreshold, vector<int> classes, at::Tensor* pProto)
	{
		at::Tensor predict = *pPredict;
		int num_class = m_vecClsNames.size();  // 类别数
		int attr_start_idx = 5 + num_class;    // mask系数或关键点起始索引
		// predict维度：(num_boxes, 5 + num_classes + extra)
		// -------------------------- 批量提取核心数据 --------------------------
		auto boxes_xywh = predict.slice(1, 0, 4).to(torch::kFloat32);         // (num_boxes, 4)：cx, cy, w, h
